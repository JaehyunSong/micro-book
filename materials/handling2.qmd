...
df %>% 
  group_by(Continent) %>%
  summarise(Politial_Right = mean(FH_PR, na.rm = TRUE),
            Civil_Liberty  = mean(FH_CL, na.rm = TRUE))
```
...
* `min()`、`max()`: 最小値と最大値
* `n()`: ケース数（引数不要）
* その他（教科書[第13.1.2章](https://www.jaysong.net/RBook/datahandling2.html#summarise%E3%81%AB%E4%BD%BF%E3%81%88%E3%82%8B%E4%BE%BF%E5%88%A9%E3%81%AA%E9%96%A2%E6%95%B0)）
...
* 理由は割愛するが、詳細は教科書[第13.2章](https://www.jaysong.net/RBook/datahandling2.html#handling2-group)を参照
...
`across()`関数を利用: 詳細は教科書[第13.1章](https://www.jaysong.net/RBook/datahandling2.html#handling2-summarise)を参照
...
`NA`でなく、**生成される列のデータ型**に応じて`NA_real_`（numeric型）、または`NA_character_`（character型）を使用（logical型なら`NA`のままでOK）
