---
title: "ミクロ政治データ分析実習"
subtitle: "第2回 RとIDE (RStudio)の導入"
...
#| echo: false
#| out-width: "250px"
knitr::include_graphics("Figs/02/rlogo.png")
```
...
#| echo: false
#| fig-height: 1
knitr::include_graphics("Figs/02/Python_logo.png")
```
...
#| echo: false
#| fig-height: 1
knitr::include_graphics("Figs/02/Julia_logo.png")
```
...
```{r}
#| echo: false
knitr::include_graphics("Figs/02/CUI_GUI.png")
```

* GUI: **G**raphical **U**ser **I**nterface; ソフトウェア上の入出力にグラフィックを利用
  * マウスでポチポチするだけで操作できる環境
  * 間違いがほぼ起きない
  * 柔軟性が低い（決まったオプション、機能しか使えない）
  * 分析の再現が難しい（出来ないわけではない）
* CUI: **C**haracter **U**ser **I**nterface
...
`Data.csv`を読み込み、`X`という変数の平均値と分散を計算
...
```{r}
#| echo: false
knitr::include_graphics("Figs/02/IDE_RStudio.png")
```
...
* ファイル・システムに関する知識（パス、相対パス、絶対パス）がない場合、プロジェクト昨日は必須
